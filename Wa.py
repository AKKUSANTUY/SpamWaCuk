...
def Tokped():
	print
	MesinTik(C+'\t SPAM TOKOPEDIA NIH CUK')
	MesinTik(W+'\t================')
	print
	number = raw_input(''+C+'MASUKKAN NOMOR TARGET CUK'+W+' ('+H+' Ex :'+C+' 0812xxxx '+W+') : ')
	jumlah = input(''+C+'JUMLAH SPAM CUK'+W+' ('+H+' Ex :'+C+' 3 '+W+') : ')
...
def Spam():
	os.system('clear')
	print(C+'Wajib FOLLOW '+W+' Bro Biar Afdoll Cuk !'+C+' :V')
	sleep(1.5)
	...
	try:
		...
		pilih = input(C+'PILIH MENU CUK'+W+' \xE2\x9E\xA4 '+C+'')
		if pilih == 1:
			Tokped() 
		elif pilih == 2:
			RupaRupa()
